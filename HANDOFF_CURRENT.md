...
- Native 只读截图可见目标卡片，但可访问性读取被 Windows 当前 URL 校验拒绝；已停止，未换通道绕过。
  待用户提供这篇文章的查看链接（仅域名和路径，不要查询参数），再补真实卡片/实体绑定；不要重复发布
  已成功文章。真实新文章自动化验收及网页/MCP 接入须按下一任务明确授权和验证，不能从本地测试推导完成。
