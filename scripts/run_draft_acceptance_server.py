...
def _delivery_platforms() -> tuple[str, ...]:
    ...
    return tuple(DELIVERY_PLATFORMS)
...
def build_acceptance_app(
    ...
    *,
    ...
    if normalized_xhs_resume_title:
        raise ValueError(
    ...
    for platform in selected:
        if platform == "zol":
            ...
            # 微博已通过标准 Content Studio 的完整 Word 草稿保存和重开核验；
            # 验收入口继续只放行已证明的 H2 与图片顺序。
            declarations[platform] = PlatformFormatDeclaration(
                platform,
                frozenset({"heading", "image_order"}),
                frozenset({2}),
    ...
    # 已从投递目录移除，不能再绕开云端实体要求；微博恢复参数也只注入
    # 当前 DRAFT-only 验收进程，生产工厂不接受该开关。
    if "zol" in selected or normalized_weibo_resume_title:
