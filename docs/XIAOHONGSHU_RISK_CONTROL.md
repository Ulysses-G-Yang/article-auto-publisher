...
11. 私密发布后半程必须在已经完成正文/图片排版验证的同一实例和 Page 中，绑定实际身份缓存、
    最终笔记标题和冻结正文。当前页面需为已观测发布页；下拉折叠回读必须
    精确为「仅自己可见」。主文档之外、重复控件、身份/标题/内容改变均不得继续点击发布。
12. 「下一步」和「发布」分别最多点击一次；并发、取消或点击超时不能重置提交守卫。单次点击后
    在有界等待内出现成功提示或同一 Page 跳转笔记管理页，返回「已提交（仅自己可见）」；
    只表示平台已接收，不表示审核通过。未收到明确接收证据返回结果未知，不自动重试。
13. 2026-09-09 用户将本轮范围限定为能发布：不再以文章 ID、管理页新实体基线、查看链接或
    后续审核/数据采集作为提交门禁。旧的只读实体核验方法保留兼容，但不进入新投递关键路径。
    完整自动化仍需单独真实验收；不能拿用户手动发布冒充代码验收。
14. 用户提供的首页 `/explore/<note_id>` 链接只作为实体 ID 的比对样本，不能替代同一卡片/账号/私密范围
    核验。查询签名不写入代码、测试、结果或日志；去掉签名是标识提取规则，不是无签名访问能力的保证。

## 5. 操作记录

- 2026-09-09 / Codex：按最新授权将私密发布接入网页计划及执行单，取消实体 ID/查看链接/后续审核
  作为发布前置要求；保留完整图文排版、身份和仅自己可见检查、单次点击。离线全量 1304 passed、
  7 skipped，真实自动化仍待一次验收；此记录时未新增真实文章，未操作 Profile/Cookie。
